...
export default function App() {
  ...
  const [selectedAgentId, setSelectedAgentId] = useState<string>('agent_sales_01');
  ...
  const currentMessages = messagesMap[selectedAgentId] || [
    {
      id: `init_${Date.now()}`,
      sender: 'assistant',
  ...
  const handleSendMessage = async (text: string) => {
    ...
    try {
      ...
      const assistantMsg: Message = {
        ...
        agentId: selectedAgent.id,
        agentName: selectedAgent.name,
        text: data.text || "Processed request via local ELAP model.",
        timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
      ...
      const fallbackMsg: Message = {
        ...
        agentId: selectedAgent.id,
        agentName: selectedAgent.name,
        text: `### ⚡ Local Synthesis (${selectedAgent.name})\n\nProcessed query **"${text}"** locally.\n\n* All data retrieved from connected RAG knowledge bases.\n* Local Ollama engine operating normally.`,
        timestamp: timestampStr
      };
      setMessagesMap(prev => ({
        ...prev,
  ...
  return (
    <div className="h-screen w-screen flex flex-col bg-[#0b0e12] text-slate-100 font-sans overflow-hidden select-none">
      ...
      <div className="flex-1 flex overflow-hidden relative">
        ...
        <main className="flex-1 flex flex-col min-w-0 bg-[#0b0f15] border-r border-[#1e293b]">
          {/* MULTI-TAB NAVIGATION BAR */}
          <div className="h-9 px-3 bg-[#0d1219] border-b border-[#1e293b] flex items-center justify-between shrink-0 select-none">
            <div className="flex items-center gap-1">
              <button
                ...
                className={`px-3 py-1 rounded-t-lg text-xs font-semibold flex items-center gap-1.5 transition-all border-t border-x ${
                  activeTab === 'chat'
                    ? 'bg-[#0b0f15] text-indigo-300 border-[#1e293b] shadow-sm'
            ...
            <div className="hidden sm:flex items-center gap-2 text-[10px] font-mono text-slate-500">
              <span>ELAP Engine: <strong className="text-emerald-400">Online</strong></span>
            </div>
          </div>
        ...
        {/* RIGHT SIDEBAR (280px) - CONTEXT & PROPERTIES */}
        <RightSidebar 
          agent={selectedAgent}
          tools={tools}
          knowledgeSources={knowledgeSources}
