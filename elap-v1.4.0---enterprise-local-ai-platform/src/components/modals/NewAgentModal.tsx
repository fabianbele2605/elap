...
export const NewAgentModal: React.FC<NewAgentModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-[#121924] border border-slate-700 rounded-2xl w-full max-w-lg overflow-hidden shadow-2xl text-slate-200">
        <div className="px-5 py-3.5 bg-[#0f141d] border-b border-slate-800 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-bold text-sm text-slate-100">Create New ELAP AI Agent</h3>
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4 text-xs font-sans">
          <div>
            ...
            <input 
              ...
              className="w-full bg-[#0c1118] border border-slate-700 rounded-lg p-2 text-slate-100 focus:outline-none focus:border-indigo-500 text-xs"
            />
          </div>

          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-slate-300 font-semibold mb-1">Domain Role</label>
              <select 
                value={role}
            ...
            <div>
              <label className="block text-slate-300 font-semibold mb-1">Local Ollama Model</label>
              <select 
                value={model}
                onChange={(e) => setModel(e.target.value)}
                className="w-full bg-[#0c1118] border border-slate-700 rounded-lg p-2 text-slate-100 focus:outline-none focus:border-indigo-500 font-mono text-xs"
              >
          ...
          <div>
            <label className="block text-slate-300 font-semibold mb-1">System Prompt / Instructions</label>
            <textarea 
              rows={3}
              value={systemPrompt}
          ...
          <div>
            <label className="block text-slate-300 font-semibold mb-1">Tool Bindings</label>
            <div className="grid grid-cols-2 gap-1.5 max-h-32 overflow-y-auto p-2 bg-[#0c1118] rounded-lg border border-slate-800 custom-scrollbar">
              {availableTools.map(t => {
                ...
                return (
                  <div 
                    ...
                    className={`p-1.5 rounded cursor-pointer flex items-center justify-between text-[11px] border transition-colors ${
                      isSelected ? 'bg-indigo-950/80 border-indigo-600 text-indigo-200' : 'bg-slate-900 border-slate-800 text-slate-400'
                    }`}
                  >
          ...
          <div className="pt-2 border-t border-slate-800 flex items-center justify-end gap-2">
            ...
            <button 
              type="submit" 
              className="px-4 py-1.5 bg-indigo-600 hover:bg-indigo-500 text-white font-medium rounded-lg text-xs shadow-md shadow-indigo-600/30"
            >
              Initialize Agent
            </button>
          </div>
